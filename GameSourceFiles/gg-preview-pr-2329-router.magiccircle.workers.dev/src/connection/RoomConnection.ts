...
type WelcomeSubscriber = (state: IState<RoomData>) => void;
...
export default class RoomConnection {
  ...
  private readonly welcomeSubscribers = new Set<WelcomeSubscriber>();
  ...
  /**
   * Subscribe to Welcome messages from the server.
   *
   * The handler will be called each time a Welcome message is received, which
   * occurs on initial connection and after reconnections. If already connected
   * when subscribing, the handler is called immediately with the current state.
   *
   * @param handler - Callback receiving the full state from the Welcome message
   * @returns Unsubscribe function
   */
  public readonly subscribeToWelcome = (handler: WelcomeSubscriber) => {
    this.welcomeSubscribers.add(handler);

    // If already connected, call handler immediately with current state
    if (this.lastRoomStateJsonable) {
      handler(this.lastRoomStateJsonable);
    }

    return () => {
      this.welcomeSubscribers.delete(handler);
    };
  };
  ...
  private onServerToClientMessage = (message: ServerToClientMessage) => {
    ...
    switch (messageType) {
      case 'Welcome': {
        ...
        // Notify welcome subscribers
        this.welcomeSubscribers.forEach((subscriber) => {
          subscriber(message.fullState);
        });
