...
import { calculateAudioProperties } from '@/Quinoa/utils/calculateAudioProperties';
import {
...
/** Default maximum distance (in tiles) for spatial audio falloff. */
const DEFAULT_MAX_AUDIO_DISTANCE = 20;

/**
 * Options for spatial sound effect playback.
 */
export interface SpatialSfxOptions {
  /**
   * Maximum distance (in tiles) at which the sound is still audible.
   * Beyond this distance, the sound will not play.
   * @default 20
   */
  maxDistance?: number;
}
...
/**
 * Plays a sound effect with automatic spatial audio based on distance from
 * the player. Volume and stereo panning are calculated from the difference
 * between the player's position and the sound source position.
 *
 * Sounds beyond `maxDistance` tiles are not played. This function reads the
 * player's current position from `positionAtom`, so it can be called from
 * anywhere without needing to pass player position explicitly.
 *
 * @param name - The sound effect name to play
 * @param sourcePosition - Grid position of the sound source (e.g., a plant)
 * @param options - Optional configuration for spatial audio behavior
 *
 * @example
 * ```ts
 * // Play a sound at a specific world position
 * playSpatialSfx('Plant_Matures', { x: 10, y: 5 });
 *
 * // Play with a custom max distance (quieter falloff)
 * playSpatialSfx('Plant_Matures', { x: 10, y: 5 }, { maxDistance: 30 });
 * ```
 */
export function playSpatialSfx(
  name: SfxName,
  sourcePosition: GridPosition,
  options?: SpatialSfxOptions
): void {
  const playerPos = get(positionAtom);
  if (!playerPos) return;

  const maxDistance = options?.maxDistance ?? DEFAULT_MAX_AUDIO_DISTANCE;
  const { volume, stereo } = calculateAudioProperties(
    playerPos,
    sourcePosition,
    maxDistance
  );
  ...
  if (volume > 0) {
    playSfx(name, { volumeMultiplier: volume, pan: stereo });
  }
}
