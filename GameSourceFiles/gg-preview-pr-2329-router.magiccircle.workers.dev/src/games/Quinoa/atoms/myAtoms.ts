 ...
 */
export function explicitlySelectItem(newSelectedIndex: number) {
  ...
  if (currentSelectedIndex === newSelectedIndex) {
    ...
    set(myLastExplicitlySelectedItemIdAtom, null);
    ...
    const selectedItem = inventoryItems[newSelectedIndex];
    if (!selectedItem) {
      console.error(
        `Selected item index ${newSelectedIndex} is out of bounds (inventory has ${inventoryItems.length} items).`
      );
      return;
    }
    const selectedItemId = getInventoryItemId(selectedItem);
    set(myLastExplicitlySelectedItemIdAtom, selectedItemId);
 ...
 */
export const myLastExplicitlySelectedItemIdAtom = atom<null | string>(null);
...
export const myValidatedSelectedItemIndexAtom = atom((get) => {
  ...
  const inventoryItems = get(myInventoryItemsAtom);
  const lastExplicitSelectedItemId = get(myLastExplicitlySelectedItemIdAtom);
  ...
  const selectedItemId = getInventoryItemId(item);
  // Only select tool if it has been explicitly selected
  // This is to prevent accidental use of tools e.g., shovel when auto-selected
  if (
    item.itemType === ItemType.Tool &&
    lastExplicitSelectedItemId !== selectedItemId
  ) {
    return null;
  }
...
export const isGardenObjectMatureAtom = atom((get) => {
  ...
  if (
    ...
    floraSpeciesDex[currentGardenObject.species].plant.harvestType ===
      HarvestType.Single
  ) {
    return true;
  }
...
export const myActiveCropMutationPetsAtom = nonPrimitiveAtom((get) => {
  ...
  const currentWeather = get(weatherAtom);
  if (!myPetSlots) {
    return [];
  }
  ...
  myPetSlots.forEach((slot) => {
    ...
    for (const ability of [
      ...
      const requiredWeather =
        'requiredWeather' in faunaAbilitiesDex[ability].baseParameters
          ? faunaAbilitiesDex[ability].baseParameters.requiredWeather
          : null;
