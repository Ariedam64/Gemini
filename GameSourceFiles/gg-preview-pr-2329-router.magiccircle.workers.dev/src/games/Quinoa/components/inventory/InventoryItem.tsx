...
const InventoryItemComponent: React.FC<InventoryItemProps> = ({
  ...
  // For plants that are still maturing, periodically update the visual.
  // We do this to show it changing in size and to update the crop count when it matures.
  ...
  useInterval(
    () => setPlantTick((t) => t + 1),
    needsPeriodicUpdate ? 3_000 : undefined
  );
  const { t } = useLingui();
    ...
    <Box position="relative">
      <Button
        ...
        onContextMenu={(e: React.MouseEvent) => {
          e.preventDefault();
          // Prevent favoriting during drag operations
