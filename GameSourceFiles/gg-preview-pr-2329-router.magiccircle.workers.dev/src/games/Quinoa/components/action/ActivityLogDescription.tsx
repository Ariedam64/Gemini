...
const PlantLogSprite: React.FC<{
  ...
  if (harvestType === HarvestType.Single) {
    ...
    growthScale = Math.max(minimumScale, slots[0].targetScale * progress);
  }
  ...
  return (
    ...
    >
      <InventorySprite item={item} size={`${scaledSize}px`} canvasScale={2} />
    </McFlex>
  );
};
...
const ActivityLogDescription: React.FC<{
  ...
  const seedSpriteSizePx = isSmallScreen ? 14 : 26;
  ...
  switch (log.action) {
    ...
    case 'FrostGranter': {
      ...
      // If the crop is Frozen, we know that it was previously Chilled or Wet and
      // turned Frozen when gaining another mutation.
      if (
        ['RainDance', 'SnowGranter'].includes(log.action) &&
        mutations.includes('Frozen')
      ) {
        const previousMutation = log.action === 'RainDance' ? 'Chilled' : 'Wet';
        text = (
          <Trans>
            {petText} made your{' '}
            <MutationText mutationId={previousMutation} isDark />{' '}
