...
import { SnowFootprintsSystem } from './systems/snow-footprints/SnowFootprintsSystem';
import { SnowballSystem } from './systems/snowball/SnowballSystem';
  ...
  implements DebugMetricsProvider, ZoomOverrideController
{
  ...
  private initializeSystems(tiledMapData: tiled.Map): void {
    ...
    // SnowFootprintsSystem - footprints layer (above ground, below world)
    const snowFootprintsSystem = new SnowFootprintsSystem(
      tiledMapData,
      this.groundContainer
    );
    this.addSystem(snowFootprintsSystem);
    ...
    // SnowballSystem - snowball throwing animations (in world container)
    const snowballSystem = new SnowballSystem(this.worldContainer);
    this.addSystem(snowballSystem);
  ...
  private onTick(_ticker: Ticker): void {
    ...
    // Render
    // Actualyl... no need to call this because it already gets called by the
    // application's ticker. TODO: Refactor
    // this.app.render();
  }
