...
import type { PetDataProvider } from '../pets/PetSystem';
import { getAvatarZLayerForTileObject } from '../tile-objects/avatarZLayer';
import {
  doesPetObscureAvatar,
...
const Config = {
  ...
  verticalOffset: AVATAR_Y_OFFSET,
  ...
  decorRotationsExemptFromPeeking: {
    WoodArch: [0, -360, 180, -180],
    StoneArch: [0, -360, 180, -180],
    MarbleArch: [0, -360, 180, -180],
    ColoredStringLights: [0, -360, 90, -90, 180, -180, 270, -270],
    StringLights: [0, -360, 90, -90, 180, -180, 270, -270],
...
export class AvatarView implements ViewportCullable {
  ...
  /** Provider for checking if pets are at the same position */
  ...
  /** Cached pet obscurance state (updated only when moving or pets change) */
  private isObscuredByPet: boolean = false;
  ...
  private constructor(
    ...
    petDataProvider: PetDataProvider,
    ...
    this.petDataProvider = petDataProvider;
    ...
    // Set tile filter for celestial area indicators (show on boardwalk/garden tiles)
    this.heldItemVisual.setTileVisibilityFilter((position) =>
      tileDataProvider.isGardenTile(position)
    );
   ...
   * @param petDataProvider - Provider to query pets at positions
   ...
   * @param camera - Camera container for position transforms
   * @returns Promise that resolves to the created AvatarView
   *
   * @example
   * ```typescript
   * const avatarView = await AvatarView.create('player-123', tileProvider, buildingProvider, avatarProvider, petProvider, uiContainer, batchRenderer, camera);
   * await avatarView.loadAvatar({ avatar, externalAvatarUrl: discordAvatarUrl });
   * ```
   */
  static async create(
  ...
  update(
    ...
    // Check for pets at the same position.
    // We need to do this every frame because pets can move onto our square.
    const petsAtThisTile = this.petDataProvider.getPetsAt(position);
    this.isObscuredByPet = petsAtThisTile.some((petSlot) =>
      doesPetObscureAvatar({ petSlot })
    );
    ...
    const shouldPeek = this.calculateShouldPeek({
      shouldSleep,
  ...
  /**
   * Determines if the avatar should play the peeking animation.
   * Peeking occurs when the avatar is obscured by something but should still be visible.
   *
   * If EITHER a pet or tile object prevents peeking, we don't peek.
   */
  private calculateShouldPeek(params: {
    shouldSleep: boolean;
    isAnotherAvatarAtThisTile: boolean;
    petsAtThisTile: PetSlot[];
    ...
    // Sleeping takes precedence over peeking
    if (shouldSleep) {
      return false;
    }
    // Always peek when sharing a tile with another avatar
    if (isAnotherAvatarAtThisTile) {
      return true;
    }
    // Must be obscured by something to trigger peeking
    if (!this.isObscuredByPet && !this.isObscuredByTile) {
      return false;
    }
    // If obscured by a pet that's too wide, don't peek
    if (this.isObscuredByPet && this.arePetsTooWideForPeeking(petsAtThisTile)) {
      return false;
    }
    // If obscured by a tile object that prevents peeking, don't peek
    if (this.isObscuredByTile && !this.isSpeciesAlwaysPeek(tileData)) {
      if (
        this.isDecorExemptFromPeeking(tileData) ||
        this.isObjectTooWideForPeeking(tileData, serverNow)
      ) {
        return false;
      }
    }
    // Obscured and nothing prevents peeking
    return true;
  }
  ...
  /**
   * Checks if any obscuring pet is too wide to prevent peeking.
   * Returns false if any obscuring pet is too wide; returns true only if all
   * obscuring pets are narrow enough to allow peeking.
   *
   * @param petsAtTile - Array of pet slots at the current tile
   * @returns True if all obscuring pets are narrow enough to trigger peeking
   */
  private arePetsTooWideForPeeking(petsAtTile: PetSlot[]): boolean {
    // If any obscuring pet is too wide, don't trigger peeking
    for (const petSlot of petsAtTile) {
      if (doesPetObscureAvatar({ petSlot })) {
        const { width } = getPetDimensions(petSlot);
        if (width > Config.maxWidthForPeeking) {
          return true; // At least one wide pet prevents peeking
        }
      }
    }
    return false; // All obscuring pets are narrow enough to allow peeking
  }
