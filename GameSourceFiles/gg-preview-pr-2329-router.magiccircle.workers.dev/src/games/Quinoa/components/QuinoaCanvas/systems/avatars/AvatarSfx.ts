...
const ANIMATION_SFX_MAP: Partial<Record<AvatarTriggerAnimationName, SfxName>> =
  {
    ...
    [AvatarTriggerAnimationName.ThrowSnowball]: 'SnowballThrow',
  };
...
export function playAnimationSfx(
  ...
  let sfxName = ANIMATION_SFX_MAP[animationName];
  if (!sfxName) return;
  ...
  // HACK: Play snow footstep when walking on snow
  if (sfxName === 'Footstep') {
    const tileIndex = gridPositionToTileIndex(
      avatarPosition.x,
      avatarPosition.y
    );
    if (isOnLayer(tileIndex, 'SnowFootprints_Indent')) {
      sfxName = 'FootstepSnow';
    }
  }
