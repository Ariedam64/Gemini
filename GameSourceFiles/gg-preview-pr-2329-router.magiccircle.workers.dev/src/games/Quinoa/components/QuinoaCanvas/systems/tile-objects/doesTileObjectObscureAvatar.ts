...
export const TileObjectObscuringEffectConfig = {
  ...
  /** Flora species that always obscure (rendered at fixed size) */
  speciesAlwaysObscure: [
    'Starweaver',
    'MoonCelestial',
    'DawnCelestial',
  ] as FloraSpeciesId[],
...
export function doesTileObjectObscureAvatar({
  ...
  // Species that always obscure (e.g., celestials) bypass size check
  if (
    tileObject.objectType === 'plant' &&
    TileObjectObscuringEffectConfig.speciesAlwaysObscure.includes(
      tileObject.species
    )
  ) {
    return true;
...
export interface PetObscuringInput {
  /** Pet slot data for the pet on the same tile */
  petSlot: PetSlot;
}

/**
 * Determines whether a pet should fade when the local player stands on the same
 * tile. Call this only when the player is on that tile.
 *
 * Pets always render behind the avatar (no z-layer check needed), so we only
 * need to check if the pet is large enough to obscure.
 */
export function doesPetObscureAvatar({ petSlot }: PetObscuringInput): boolean {
  const maxSize = getPetMaxPixelSize(petSlot);
  return maxSize >= TileObjectObscuringEffectConfig.minPixelSizeToObscure;
}

/**
 * Gets the dimensions (width and height) of a pet.
 *
 * @param petSlot - The pet slot data
 * @returns Object containing width and height
 */
export function getPetDimensions(petSlot: PetSlot): {
  width: number;
  height: number;
} {
  const blueprint = faunaSpeciesDex[petSlot.petSpecies];
  if (!blueprint) {
    return { width: 0, height: 0 };
  }
  const texture = getTextureFromTileRef(blueprint.tileRef);
  const scale =
    getPetScale({
      speciesId: petSlot.petSpecies,
      xp: petSlot.xp,
...
/**
 * Gets the max pixel size of a pet.
 */
function getPetMaxPixelSize(petSlot: PetSlot): number {
  const { width, height } = getPetDimensions(petSlot);
  return Math.max(width, height);
}
