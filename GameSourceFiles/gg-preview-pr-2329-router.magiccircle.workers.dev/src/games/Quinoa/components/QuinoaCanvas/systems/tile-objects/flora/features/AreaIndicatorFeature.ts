...
/**
 * Filter function to determine if a tile should show the area indicator.
 */
export type TileVisibilityFilter = (position: GridPosition) => boolean;

/** Sprite with its tile offset for visibility filtering. */
interface AreaIndicatorSprite {
  sprite: Sprite;
  dx: number;
  dy: number;
}
...
/**
 * Area indicator that filters visibility per-tile based on a filter function.
 * Used for held items where indicators should only show on valid tiles
 * (e.g., garden or boardwalk tiles).
 */
export class FilteredAreaIndicator {
  public readonly container: Container;
  private readonly sprites: AreaIndicatorSprite[] = [];

  constructor(abilityId: FloraAbilityId) {
    const ability = floraAbilitiesDex[abilityId];
    const { activationTileRef, tileRadius } = ability.baseParameters;

    this.container = new Container({ label: 'FilteredAreaIndicator' });
    ...
    for (const { dx, dy } of offsets) {
      ...
      sprite.position.set(dx * TILE_SIZE_WORLD, dy * TILE_SIZE_WORLD);
      sprite.visible = false; // Start hidden, updateVisibility() controls this
      this.container.addChild(sprite);
      this.sprites.push({ sprite, dx, dy });
  ...
  /**
   * Updates sprite visibility based on player position and tile filter.
   *
   * @param playerPosition - Current player grid position
   * @param filter - Function that returns true if a tile should show the indicator
   */
  updateVisibility(
    playerPosition: GridPosition,
    filter: TileVisibilityFilter
  ): void {
    for (const { sprite, dx, dy } of this.sprites) {
      const tilePosition: GridPosition = {
        x: playerPosition.x + dx,
        y: playerPosition.y + dy,
      };
      sprite.visible = filter(tilePosition);
    }
  }
...
export class AreaIndicatorFeature implements PlantVisualFeature {
  ...
  constructor(context: PlantVisualFeatureContext) {
    ...
    const ability = floraAbilitiesDex[abilityId];
    const { requiredWeather } = ability.baseParameters;

    if (!requiredWeather) {
      throw new Error(
        'AreaIndicatorFeature requires an ability with requiredWeather'
      );
    }
