...
export interface TileObjectDataProvider {
  ...
  /**
   * Checks if a tile is a valid target for celestial abilities.
   * Returns true if the tile is a boardwalk tile OR a garden/dirt tile.
   *
   * @param position - Grid position to check
   * @returns true if the tile is a valid celestial target
   */
  isGardenTile(position: GridPosition): boolean;
}
...
export class TileObjectSystem implements TileObjectDataProvider, QuinoaSystem {
   ...
   * @returns true if the tile is a valid celestial target
   */
  public isGardenTile(position: GridPosition): boolean {
    const globalIdx = getGlobalTileIndexFromCoordinate(
      this.map,
      position.x,
      position.y
    );
    // Check if it's a boardwalk tile
    if (this.map.globalTileIdxToBoardwalk[globalIdx] !== undefined) {
      return true;
    }
    // Check if it's a garden/dirt tile
    if (this.map.globalTileIdxToDirtTile[globalIdx] !== undefined) {
      return true;
    }
    return false;
  }
