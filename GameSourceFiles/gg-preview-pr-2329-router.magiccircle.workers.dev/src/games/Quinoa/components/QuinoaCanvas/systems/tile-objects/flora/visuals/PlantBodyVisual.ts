...
export class PlantBodyVisual {
  ...
  constructor(private readonly options: PlantBodyVisualOptions) {
    this.isolateRendering = options.isolateRendering ?? false;
    this.normalizeScale = options.normalizeScale ?? false;
  ...
  update(serverTime: number): void {
    ...
    if (this.plantBaseSprite) {
      if (
        ...
        const scale = this.normalizeScale ? getNormalizedScale(1) : 1;
        this.plantBaseSprite.scale.set(scale);
      } else {
        const plantProgress = getElasticProgress(
          plant.plantedAt,
          plant.maturedAt,
          serverTime
        );
        let scale = PLANT_MIN_SCALE + (1 - PLANT_MIN_SCALE) * plantProgress;
  ...
  private createCrops(): void {
    ...
    for (const slotConfig of slotConfigs) {
      ...
      const crop = new GrowingCropVisual({
        ...
        slotState,
        disableWobbleRotation: this.isolateRendering,
        normalizeScale: this.normalizeScale,
        worldPosition: this.options.worldPosition,
      });
