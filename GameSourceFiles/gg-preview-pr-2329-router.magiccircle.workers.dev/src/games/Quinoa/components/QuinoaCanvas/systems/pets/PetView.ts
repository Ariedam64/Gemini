...
export class PetView {
  ...
  update(
    ...
    // 6. Update obscuring opacity (fade when player is on same tile)
  ...
  /**
   * Updates opacity based on whether the local player is on the same tile.
   * Makes large pets semi-transparent so the avatar remains visible.
   */
  private updateObscuringOpacity(context: QuinoaFrameContext): void {
    ...
    this.displayObject.alpha =
      isLocalPlayerOnSameTile && doesPetObscureAvatar({ petSlot: this.petSlot })
