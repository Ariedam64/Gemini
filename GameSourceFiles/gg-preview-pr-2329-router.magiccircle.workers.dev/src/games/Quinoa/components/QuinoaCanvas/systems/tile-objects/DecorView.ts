...
export class DecorView implements GardenObjectView {
  ...
  constructor(decorObject: DecorObject, worldPosition: GridPosition) {
    ...
    this.displayObject = this.decorVisual.container;
  ...
  private addFeatures(): void {
    const context = {
      ...
      baseContainer: this.decorVisual.container,
    ...
    for (const FeatureClass of DECOR_FEATURES) {
      if (FeatureClass.shouldCreate(context)) {
        ...
        // Feature's displayObject is already added as child of baseContainer
  ...
  /**
   * Destroy the decor visual and its container.
   */
  destroy(): void {
    for (const feature of this.features) {
      feature.destroy();
