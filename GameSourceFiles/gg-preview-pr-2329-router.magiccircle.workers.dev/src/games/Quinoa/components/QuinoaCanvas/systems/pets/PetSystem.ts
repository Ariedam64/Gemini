...
/**
 * Interface for querying pet data at positions.
 * Used by AvatarView to check for pets that might obscure the avatar.
 */
export interface PetDataProvider {
  /** Returns all pet slots at a given grid position. */
  getPetsAt(gridPosition: GridPosition): PetSlot[];
}
 ...
 */
export class PetSystem implements QuinoaSystem, PetDataProvider {
  ...
  /** Cached pet infos for position lookups */
  private petInfos: PetInfo[] = [];
  ...
  private onPetDataChanged(petInfos: PetInfo[]): void {
    // Cache pet infos for position lookups
  ...
  /**
   * Returns all pet slots at a given grid position.
   * Implements PetDataProvider.
   */
  getPetsAt(gridPosition: GridPosition): PetSlot[] {
    const petsAtPosition: PetSlot[] = [];
    for (const petInfo of this.petInfos) {
      if (
        petInfo.position.x === gridPosition.x &&
        petInfo.position.y === gridPosition.y
      ) {
        petsAtPosition.push(petInfo.slot);
      }
    }
    return petsAtPosition;
  }
}
