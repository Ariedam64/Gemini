...
import { PlanterPotFeature } from '../features/PlanterPotFeature';
import type {
  PlantVisualFeature,
...
export interface PlantVisualOptions {
  ...
  /**
   * When true, renders a planter pot.
   * Defaults to false.
   */
  renderPlanterPot?: boolean;
  /**
   * When true, normalizes scale values using getNormalizedScale for better
   * visual appearance in inventory/UI contexts.
   * Defaults to false.
   */
...
export class PlantVisual {
  ...
  private readonly renderPlanterPot: boolean;
  ...
  constructor(options: PlantVisualOptions) {
    ...
    this.renderPlanterPot = renderPlanterPot;
    ...
    this.bodyVisual = new PlantBodyVisual({
      plant,
      blueprint,
      forceMaxScale,
      isolateRendering,
  ...
  private addFeatures(): void {
    const context: PlantVisualFeatureContext = {
      ...
      renderPlanterPot: this.renderPlanterPot,
    };
