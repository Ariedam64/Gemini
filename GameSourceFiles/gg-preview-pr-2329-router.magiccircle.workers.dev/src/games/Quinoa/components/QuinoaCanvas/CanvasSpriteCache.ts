import { Container, isMobile, Rectangle, type Renderer, Sprite } from 'pixi.js';
...
const MAX_CANVAS_CACHE_SIZE =
  isRunningInsideDiscord && isMobile.any ? 64 : isMobile.any ? 96 : 128; // For UI rendering (inventory, menus, etc.)
...
/** Generates state suffix for cache keys (d=disabled, u=unknown, p=unpremultiply, r=renderPlanterPot). */
function getStateSuffix(options: CacheKeyOptions): string {
  const {
    isDisabled = false,
    isUnknown = false,
    unpremultiply = false,
    renderPlanterPot = false,
  } = options;
  return `${isDisabled ? ':d' : ''}${isUnknown ? ':u' : ''}${unpremultiply ? ':p' : ''}${renderPlanterPot ? ':r' : ''}`;
}
