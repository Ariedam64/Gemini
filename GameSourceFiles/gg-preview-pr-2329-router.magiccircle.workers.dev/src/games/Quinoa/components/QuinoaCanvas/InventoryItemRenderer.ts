...
import { Container, Graphics, isMobile, Sprite } from 'pixi.js';
 ...
 * On memory-constrained devices (including iPhone 11), we use a lower scale to
 * out-of-memory crashes.
 */
// const INVENTORY_SIZE_SCALE = isMobile.phone ? 1 : 1.5;
let INVENTORY_SIZE_SCALE = 1.5;
if (isMobile.any) {
  INVENTORY_SIZE_SCALE = 0.75;
}
if (isMobile.any && isRunningInsideDiscord) {
  INVENTORY_SIZE_SCALE = 0.5;
}
...
interface PlantCanvasOptions {
  plant: PlantInventoryItem;
  serverNow: number;
  renderPlanterPot?: boolean;
}
...
export function getInventoryItemCanvas(
  ...
  renderPlanterPot: boolean = false
): HTMLCanvasElement {
...
function renderPlantCanvas(options: PlantCanvasOptions): HTMLCanvasElement {
  const { plant, serverNow, renderPlanterPot = false } = options;
  const blueprint = floraSpeciesDex[plant.species];
  // Render full plant with crops, centered on visual bounds
  const renderer = getCanvasCache().renderer;
  ...
  // Get actual bounds of the entire visual (plant + crops + pot)
