...
const PurchasableShopItem = <T extends ShopItem>({
  ...
  const getItemData = () => {
    switch (purchaseItem.itemType) {
      case ItemType.Seed: {
        ...
        const expiryDate = 'expiryDate' in seed && seed.expiryDate;
        const description =
          'abilities' in plant
      ...
      case ItemType.Egg: {
        const eggBlueprint = EggsDex[purchaseItem.eggId];
        const { name, coinPrice, tileRef, rarity } = eggBlueprint;
        const expiryDate =
          'expiryDate' in eggBlueprint && eggBlueprint.expiryDate;
        const item: InventoryItem = {
  ...
  const getButtonBg = () => {
    if (itemData.expiryDate) {
      if (isAvailable) {
        return '#285080'; // Brighter dark blue
      }
      if (isUnavailable) {
        return '#1a344a'; // Darker blue
      }
      return '#3a3d44'; // Slightly lighter gray for out of stock
    }
    if (isAvailable) {
      return 'Brown.Magic';
    }
            ...
            >
              {itemData.expiryDate && (
                <McFlex auto gap={0.5} col orient="top right">
                  ...
                  >
                    <ShopItemBadgeChip bg="Blue.Magic" color="MagicWhite">
                      <Trans>WINTER EXCLUSIVE!</Trans>
                    </ShopItemBadgeChip>
                  </McTooltip>
