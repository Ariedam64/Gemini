...
export const getAbilityTriggerDescription = (
  ...
  switch (abilityId) {
    ...
    case 'SnowGranter': {
      const { growSlot } = data as PetAbilityToastData['SnowGranter'];
      const { species, mutations } = growSlot;
      const { name: cropName } = floraSpeciesDex[species].crop;
      // If the crop is Frozen, we know that it was previously Wet and
      // turned Frozen when gaining Chilled from Snow Granter.
      if (mutations.includes('Frozen')) {
        return (
          <Trans>
            {name} made your <MutationText mutationId="Wet" isDark /> {cropName}{' '}
            turn <MutationText mutationId="Frozen" isDark />!
          </Trans>
        );
      }
      return (
        <Trans>
          {name} made your {cropName} turn{' '}
          <MutationText mutationId="Chilled" isDark />!
        </Trans>
      );
    }
    case 'FrostGranter': {
      const { growSlot } = data as PetAbilityToastData['FrostGranter'];
      const { name: cropName } = floraSpeciesDex[growSlot.species].crop;
      return (
        <Trans>
          {name} made your {cropName} turn{' '}
    ...
    case 'DawnKisser':
      return;
    default: {
      const exhaustiveCheck: never = abilityId;
...
export const getAbilityDescription = (
  ...
  switch (abilityId) {
    ...
    case 'SnowyCropSizeBoost':
      return <Trans>Chance to increase size of garden crops during Snow</Trans>;
    ...
    case 'SnowyPlantGrowthBoost':
      return (
        <Trans>Chance to reduce growth time of garden plants during Snow</Trans>
      );
    ...
    case 'SnowyCropMutationBoost':
      return (
        <Trans>
          Increases chance of garden crops gaining mutations during Snow
        </Trans>
      );
    ...
    case 'SnowyHungerRestore':
      return (
        <Trans>Chance to restore hunger of an active pet during Snow</Trans>
      );
    ...
    case 'SnowyHungerBoost':
      return (
        <Trans>Reduces hunger depletion rate of active pets during Snow</Trans>
      );
    ...
    case 'SnowyPetXpBoost':
      return <Trans>Chance to give XP to active pets during Snow</Trans>;
    ...
    case 'SnowyCoinFinder':
      return <Trans>Chance to find coins in your garden during Snow</Trans>;
    ...
    case 'SnowyEggGrowthBoost':
      return (
        <Trans>Chance to reduce hatch time of garden eggs during Snow</Trans>
      );
    ...
    case 'SnowGranter':
      return (
        <>
          <Trans>Chance to grant</Trans> <MutationText mutationId="Chilled" />{' '}
          <Trans>mutation to a garden crop</Trans>
        </>
      );
    ...
    case 'FrostGranter':
      return (
        <>
          <Trans>Chance to grant</Trans> <MutationText mutationId="Frozen" />{' '}
          <Trans>mutation to a garden crop</Trans>
        </>
      );
