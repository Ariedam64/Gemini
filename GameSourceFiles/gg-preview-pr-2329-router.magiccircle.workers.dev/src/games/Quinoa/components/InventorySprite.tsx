...
interface InventorySpriteProps {
  ...
  canvasScale?: number;
  /** Whether to render a planter pot for plants (defaults to false). */
...
const InventorySprite: React.FC<InventorySpriteProps> = ({
  ...
  const visualKey = useMemo(() => {
    return generateInventoryItemCacheKey(item, {
      isUnknown,
      renderPlanterPot,
    });
  }, [item, isUnknown, renderPlanterPot]);

  const itemScale = useMemo(() => {
    switch (item.itemType) {
      case ItemType.Produce:
        return Math.max(minimumCropScale, getNormalizedScale(item.scale));
      case ItemType.Pet: {
  ...
  useEffect(() => {
    ...
    const canvas = getInventoryItemCanvas(item, isUnknown, renderPlanterPot);
