...
type WelcomeHandler<TContext> = (context: TContext) => void;
...
export class PatchRouter<TContext = void> {
  ...
  private welcomeHandler?: WelcomeHandler<TContext>;

  /**
   * Register a handler for Welcome messages (initial full state).
   *
   * This handler is invoked when `processWelcome` is called, typically when
   * the server sends the initial state on connection or reconnection.
   *
   * @param handler - Callback receiving the full state context
   */
  onWelcome(handler: WelcomeHandler<TContext>): void {
    this.welcomeHandler = handler;
  }

  /**
   * Process a Welcome message by invoking the registered welcome handler.
   *
   * Call this when the server sends the initial full state (Welcome message).
   *
   * @param context - The full state context from the Welcome message
   */
  processWelcome(context: TContext): void {
    this.welcomeHandler?.(context);
  }
