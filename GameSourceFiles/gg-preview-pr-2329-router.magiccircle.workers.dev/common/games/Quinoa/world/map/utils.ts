...
export function tileSize(): number {
  return quinoaMapJson.tilewidth;
}

export function gridPositionToTileIndex(x: number, y: number) {
  return x + y * quinoaMapJson.width;
}

export function tileIndexToGridPosition(tileIndex: number): {
  x: number;
  y: number;
} {
  const cols = quinoaMapJson.width;
  return {
    x: tileIndex % cols,
...
export function tileIndexToPixelPosition(tileIndex: number): {
  x: number;
  y: number;
} {
  const { x, y } = tileIndexToGridPosition(tileIndex);
  const tileSize = quinoaMapJson.tilewidth;
  return {
    x: x * tileSize,
    y: y * tileSize,
  };
...
export function isPresentOnMapLayer(
  tileIndex: number,
  layerName: string
): boolean {
  const layer = quinoaMapJson.layers.find(
...
export function getTileIndexSet(layerName: string): Set<number> {
  const layer = quinoaMapJson.layers.find(
    (l) => l.type === 'tilelayer' && l.name === layerName
  );
  if (!layer || !layer.data) {
  ...
  return new Set(
    layer.data.map((t, i) => (t > 0 ? i : -1)).filter((i) => i !== -1)
  );
}

const layerNameToTileIndexSet: Record<string, Set<number>> = {};

export function isOnLayer(tileIndex: number, layerName: string): boolean {
  let tileIndexSet = layerNameToTileIndexSet[layerName];
  if (!tileIndexSet) {
    tileIndexSet = getTileIndexSet(layerName);
    layerNameToTileIndexSet[layerName] = tileIndexSet;
  }
  return tileIndexSet.has(tileIndex);
}
