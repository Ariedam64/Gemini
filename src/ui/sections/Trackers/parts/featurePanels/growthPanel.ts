...
export const growthPanel: FeaturePanelDefinition = {
    ...
    /**
     * Render team-aggregated stats for grouped pets (2-3 with identical abilities)
     * Shows combined team multiplier with same layout as individual cards (BOOST, NEXT, ALL)
     */
    renderGroupedSlot: (pets: UnifiedPet[], team: PetTeam, container: HTMLElement, viewType?: string) => {
        const garden = Globals.myGarden.get();
        const now = Date.now();

        // Calculate TEAM boost (combined from all pets)
        ...
        // 2. NEXT Row (team multiplier with next item sprite)
        if ((showEgg || isSummaryView) && hasEgg) {
            const eggNext = findNextReadyEgg(growingEggs, now);
            const eggPercent = growingEggs.length > 0
        ...
        // 3. ALL Row (total count with stacked sprites)
        if ((showEgg || isSummaryView) && hasEgg) {
            const allEggPercent = growingEggs.length > 0
                ? calcAvgPercentEggs(growingEggs, now, eggMultiplier)
                : 100;
