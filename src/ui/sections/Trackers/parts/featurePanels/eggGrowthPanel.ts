/**
 * Egg Growth Timer Panel
 *
 * Feature panel for displaying egg growth timers.
...
// ─────────────────────────────────────────────────────────────────────────────
// Egg Growth Panel Component
// ─────────────────────────────────────────────────────────────────────────────

class EggGrowthTimerPanel {
    ...
    constructor(options: { teamId: string }) {
        ...
        this.root.className = 'growth-timer-panel egg-growth';
    }
    ...
    refresh(): void {
        const timers = MGGrowthTimers.getEggTimers(this.teamId);
        const stats = MGGrowthTimers.getBoostStats(this.teamId, 'egg');
    ...
    private updateTimers(timers: GrowthTimer[]): void {
        ...
        if (timers.length === 0) {
            this.listContainer.innerHTML = '<div class="empty-state">No Growing Eggs...</div>';
            return;
        }
    ...
    private getSpriteHtml(species: string): string {
        ...
        try {
            ...
            console.warn(`[EggGrowthPanel] Failed to render sprite for ${species}:`, error);
        }
...
export const eggGrowthPanel: FeaturePanelDefinition = {
    id: 'egg-growth',
    label: 'Egg Growth',
    icon: '🥚',
    category: 'tracking',
    ...
    getSummary: (team, pets) => {
        const timers = MGGrowthTimers.getEggTimers(team.id);
        ...
        return {
            text: `${hours}h`,
            variant: hours < 6 ? 'high' : hours < 24 ? 'medium' : 'low',
            tooltip: `Next egg in ${hours} hours`,
            priority: 8,
        };
    },

    buildPanel: (team, container) => {
        const panel = new EggGrowthTimerPanel({ teamId: team.id });
    ...
    renderPetSlot: (pet: UnifiedPet, team: PetTeam, container: HTMLElement) => {
        const ability = pet.abilities.find(a => a.includes('EggGrowth'));
        if (!ability) {
            container.innerHTML = '<div class="stat-row"><span class="stat__label">BOOST</span><span class="stat__value">None</span></div>';
            return;
        }

        // Parse ability for value (e.g. EggGrowthI, EggGrowthII)
        const isTier2 = ability.endsWith('II');
        const minutes = isTier2 ? 30 : 15; // Assumption based on common tiers
    ...
    shouldDisplay: (team, pets) => {
        // Only show if team has egg growth boost abilities
        return pets.some(p =>
