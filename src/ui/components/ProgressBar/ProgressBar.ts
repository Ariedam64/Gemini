 ...
 * - Logic file: ProgressBar.ts
 * - Style file: progressBar.css.ts
 */
...
export interface ProgressBarHandle extends HTMLDivElement {
 ...
 */
export function ProgressBar(opts: ProgressBarOptions): ProgressBarHandle {
    ...
    const fill = element("div", {
        className: `progress-fill ${progressStyle.className}`.trim()
    }) as HTMLDivElement;
    ...
    // Handle API
    const handle = row as ProgressBarHandle;
    ...
    handle.setPercentage = (value: number) => {
        ...
        fill.className = `progress-fill ${style.className}`.trim();
