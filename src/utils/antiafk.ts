...
export function createAntiAfkController() {
  ...
  function unswallowAll() {
    for (const { t, h, target } of listeners) {
      try {
        target.removeEventListener(t as any, h, { capture: true } as any);
      } catch {}
    }
    listeners.length = 0;
  }
  ...
  function restoreProps() {
    try {
      if (saved.hidden)
        Object.defineProperty(docProto, "hidden", saved.hidden);
    } catch {}
    try {
      if (saved.visibilityState)
        Object.defineProperty(docProto, "visibilityState", saved.visibilityState);
    } catch {}
    try {
      if (saved.hasFocus) (document as any).hasFocus = saved.hasFocus;
    } catch {}
  }
...
export const antiAfk = createAntiAfkController();