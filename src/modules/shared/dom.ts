/**
 * DOM utilities for Phase 2 features
 * MutationObserver helpers for detecting element addition/removal
 * Based on QPM's dom.ts pattern
 */
 ...
 */
export function onAdded(selector: string, callback: (element: Element) => void): ObserverHandle {
 ...
 */
export function onRemoved(selector: string, callback: (element: Element) => void): ObserverHandle {
    const observer = new MutationObserver((mutations) => {
        for (const mutation of mutations) {
