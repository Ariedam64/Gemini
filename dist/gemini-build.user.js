...
`,i1='<svg width="12" height="12" viewBox="0 0 24 24" fill="currentColor"><path d="M12 21.35l-1.45-1.32C5.4 15.36 2 12.28 2 8.5 2 5.42 4.42 3 7.5 3c1.74 0 3.41.81 4.5 2.09C13.09 3.81 14.76 3 16.5 3 19.58 3 22 5.42 22 8.5c0 3.78-3.4 6.86-8.55 11.54L12 21.35z"/></svg>',s1='<svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M12 21.35l-1.45-1.32C5.4 15.36 2 12.28 2 8.5 2 5.42 4.42 3 7.5 3c1.74 0 3.41.81 4.5 2.09C13.09 3.81 14.76 3 16.5 3 19.58 3 22 5.42 22 8.5c0 3.78-3.4 6.86-8.55 11.54L12 21.35z"/></svg>';function l1(e){const{species:t,itemCount:n,isFavorited:r,isMobile:o,onClick:a}=e,i=b("button",{className:`gemini-qol-bulkFavorite-btn${o?" mobile":""}`,title:`${r?"Unfavorite":"Favorite"} all ${n} ${t}`});return i.dataset.species=t,i.appendChild(c1(t,o)),i.appendChild(d1(r)),i.appendChild(u1(t)),i.addEventListener("click",async s=>{s.preventDefault(),s.stopPropagation(),a();}),i}function c1(e,t){try{if(!W.isReady()||!W.has("plant",e))return $l(e);const n=t?YC:qC,r=W.toCanvas("plant",e,{scale:n});return r.className="gemini-qol-bulkFavorite-sprite",r}catch(n){return console.warn(`[BulkFavorite] Failed to load sprite for ${e}:`,n),$l(e)}}function $l(e){return b("div",{className:"gemini-qol-bulkFavorite-fallback"},e.charAt(0).toUpperCase())}function d1(e){const t=b("span",{className:`gemini-qol-bulkFavorite-heart ${e?"filled":"outline"}`});return t.innerHTML=e?i1:s1,t}function u1(e){return b("span",{className:"gemini-qol-bulkFavorite-label"},e)}let lt=null,ct=null,st=null,ao=false,Wn=null,Fn=false,rn=null;const ii=[];function Xr(e){ii.push(e);}function p1(){for(const e of ii)try{e();}catch(t){console.warn("[BulkFavorite] Cleanup error:",t);}ii.length=0;}function np(){return window.innerWidth<=tp}function f1(e){return new Promise(t=>setTimeout(t,e))}function rp(){if(ao)return;if(document.getElementById(ai)){ao=true;return}const e=document.createElement("style");e.id=ai,e.textContent=a1,document.head.appendChild(e),ao=true;}function g1(){document.getElementById(ai)?.remove(),ao=false;}function m1(){const e=Ht().get();if(!e?.items)return [];const t=new Set(e.favoritedItemIds??[]),n=new Map;for(const o of e.items){const a=o;if(a.itemType!=="Produce")continue;const i=a.species,s=a.id;if(!i||!s)continue;const c=n.get(i);c?c.push(s):n.set(i,[s]);}const r=[];for(const[o,a]of n){const i=a.length>0&&a.every(s=>t.has(s));r.push({species:o,itemIds:a,allFavorited:i});}return r.sort((o,a)=>o.species.localeCompare(a.species)),r}async function h1(e){const t=Ht().get();if(!t?.items)return;const n=new Set(t.favoritedItemIds??[]),r=[];for(const i of t.items){const s=i;if(s.itemType!=="Produce"||s.species!==e)continue;const c=s.id;c&&r.push({id:c,favorited:n.has(c)});}if(r.length===0)return;const o=r.every(i=>i.favorited),a=o?r.filter(i=>i.favorited):r.filter(i=>!i.favorited);console.log(`🔄 [BulkFavorite] ${o?"Unfavoriting":"Favoriting"} ${a.length}/${r.length} ${e}`);for(const i of a)Bo(i.id),await f1(t1);}function si(e,t){const{species:n,itemIds:r,allFavorited:o}=e;return l1({species:n,itemCount:r.length,isFavorited:o,isMobile:t,onClick:()=>h1(n)})}function b1(e){const t=b("div",{id:Ln}),n=e.getBoundingClientRect(),r=Math.max(window.innerHeight-ZC,e1);return t.style.maxHeight=`${r}px`,t.style.position="fixed",t.style.left=`${n.right+QC}px`,t.style.top=`${n.top}px`,t}function Bl(e,t,n){const r=b("div",{id:e,className:"gemini-qol-bulkFavorite-row"}),o=t.getBoundingClientRect();return n==="top"?r.style.bottom=`${window.innerHeight-o.top+oi}px`:r.style.top=`${o.bottom+oi}px`,r.style.left=`${o.left}px`,r.style.maxWidth=`${o.width}px`,r}function Dl(){const e=m1();np()?y1(e):x1(e);}function x1(e){if(lt){if(lt.innerHTML="",e.length===0){lt.style.display="none";return}lt.style.display="flex";for(const t of e)lt.appendChild(si(t,false));console.log(`🎯 [BulkFavorite] Desktop: Rendered ${e.length} produce types`);}}function y1(e){if(!ct||!st)return;if(ct.innerHTML="",st.innerHTML="",e.length===0){ct.style.display="none",st.style.display="none";return}ct.style.display="flex";const t=e.slice(0,Fl),n=e.slice(Fl);for(const r of t)ct.appendChild(si(r,true));if(n.length>0){st.style.display="flex";for(const r of n)st.appendChild(si(r,true));}else st.style.display="none";console.log(`🎯 [BulkFavorite] Mobile: Rendered ${e.length} types`);}function v1(){const e=document.querySelector(".McGrid");if(!e)return console.log("⚠️ [BulkFavorite] No .McGrid found"),null;const t=e.getBoundingClientRect();if(window.innerWidth<=tp)return console.log(`🎯 [BulkFavorite] Mobile mode - using McGrid: ${Math.round(t.width)}x${Math.round(t.height)}`),e;const r=window.innerWidth/2;let o=null,a=0;const i=e.querySelectorAll(".McFlex, .McGrid");for(const s of i){const c=s.getBoundingClientRect();if(c.width<200||c.height<200||c.width>window.innerWidth-100)continue;const d=c.left+c.width/2,l=1-Math.abs(d-r)/r,p=c.width*c.height*l;p>a&&(o=s,a=p);}if(o){const s=o.getBoundingClientRect();return console.log(`🎯 [BulkFavorite] Found desktop container: ${Math.round(s.width)}x${Math.round(s.height)} @ (${Math.round(s.left)}, ${Math.round(s.top)})`),o}return console.log(`🎯 [BulkFavorite] Fallback to McGrid: ${Math.round(t.width)}x${Math.round(t.height)}`),e}let on=null;function li(){on&&clearTimeout(on),on=setTimeout(()=>{w1();},n1);}function w1(){const e=v1();if(!e)return;console.log("🎯 [BulkFavorite] Found inventory modal container"),Vn(),rp(),Wn=e,np()?(ct=Bl(r1,e,"top"),st=Bl(o1,e,"bottom"),document.body.appendChild(ct),document.body.appendChild(st)):(lt=b1(e),document.body.appendChild(lt)),Dl(),rn&&rn(),rn=Ht().subscribeFavorites(()=>{Fn&&Dl();});}function Vn(){on&&(clearTimeout(on),on=null),rn&&(rn(),rn=null),lt?.remove(),lt=null,ct?.remove(),ct=null,st?.remove(),st=null,Wn=null;}function S1(){Vn();}async function ci(){if(!pr().enabled){console.log("⚠️ [BulkFavorite] Feature disabled");return}rp();const t=await Ai.onChangeNow(o=>{const a=o==="inventory";a!==Fn&&(Fn=a,a?li():Vn());}),n=XC(".McGrid",()=>{Fn&&(lt||ct||li());}),r=KC(".McGrid",o=>{Wn&&Wn===o&&Vn();});Xr(()=>t()),Xr(()=>n.disconnect()),Xr(()=>r.disconnect()),Xr(()=>{Vn(),Fn=false,Wn=null;}),console.log("✅ [BulkFavorite] Started (State-driven + DOM observation)");}function di(){p1(),g1(),console.log("🛑 [BulkFavorite] Stopped");}function C1(e){const t=pr();t.enabled=e,e?ci():di();}let Kr=false;const k1={init(){Kr||(ci(),Kr=true);},destroy(){Kr&&(di(),Kr=false);},isEnabled(){return Qd()},renderButton:li,removeButton:S1,startWatching:ci,stopWatching:di,setEnabled:C1};function P1(e){e.logStep("WebSocket","Capturing WebSocket...");let t=null;return t=Rd(n=>{n.ws&&(e.logStep("WebSocket","WebSocket captured","success"),t?.(),t=null);},{intervalMs:250}),VC({debug:false}),()=>{t?.(),t=null;}}async function T1(e){e.logStep("Atoms","Prewarming Jotai store...");try{await Ph(),await wh({timeoutMs:8e3,intervalMs:50}),e.logStep("Atoms","Jotai store ready","success");}catch(t){e.logStep("Atoms","Jotai store not captured yet","error"),console.warn("[Bootstrap] Jotai store wait failed",t);}}function A1(e){e.logStep("Globals","Initializing global variables...");try{Ld(),e.logStep("Globals","Global variables ready","success");}catch(t){e.logStep("Globals","Failed to initialize global variables","error"),console.warn("[Bootstrap] Global variables init failed",t);}}function I1(e){e.logStep("API","Exposing Gemini API...");try{ZS(),e.logStep("API","Gemini API ready","success");}catch(t){e.logStep("API","Failed to expose API","error"),console.warn("[Bootstrap] API init failed",t);}}function wa(){return new Promise(e=>{typeof requestIdleCallback<"u"?requestIdleCallback(()=>e(),{timeout:50}):setTimeout(e,0);})}async function _1(e){e.logStep("HUD","Loading HUD preferences..."),await wa();const t=FC();await wa();const n=await LC({hostId:"gemini-hud-root",initialWidth:t.width,initialOpen:t.isOpen,onWidthChange:r=>Hr("width",r),onOpenChange:r=>Hr("isOpen",r),themes:Nn,initialTheme:t.theme,onThemeChange:r=>Hr("theme",r),buildSections:r=>tC({applyTheme:r.applyTheme,initialTheme:r.initialTheme,getCurrentTheme:r.getCurrentTheme,setHUDWidth:r.setHUDWidth,setHUDOpen:r.setHUDOpen}),initialTab:t.activeTab,onTabChange:r=>Hr("activeTab",r)});return await wa(),e.logStep("HUD","HUD ready","success"),n}async function E1(e){e.setSubtitle("Activating Gemini modules...");const t=7;let n=0;await Pd(r=>{r.status==="success"?(n++,e.logStep("Modules",`Loading modules... (${n}/${t})`)):r.status==="error"&&e.logStep("Modules",`Loading modules... (${n}/${t}) - ${r.name} failed`,"error");}),e.logStep("Modules",`All modules loaded (${t}/${t})`,"success");}async function M1(e){try{W.isReady()||await W.init(),oe.resolveSprites();const t=[],n=oe.get("plants");if(n)for(const a of Object.values(n))a?.seed?.spriteId&&t.push(a.seed.spriteId),a?.plant?.spriteId&&t.push(a.plant.spriteId),a?.crop?.spriteId&&t.push(a.crop.spriteId);const r=oe.get("pets");if(r)for(const a of Object.values(r))a?.spriteId&&t.push(a.spriteId);const o=[...new Set(t)];o.length>0&&await W.warmup(o,()=>{},5);}catch(t){console.warn("[Bootstrap] Sprite warmup failed",t);}}async function L1(e){e.logStep("Sections","Preloading UI sections...");try{await nC(),e.logStep("Sections","Sections preloaded","success");}catch(t){e.logStep("Sections","Sections preload failed","error"),console.warn("[Bootstrap] Sections preload failed",t);}}function F1(e){e.logStep("Features","Initializing features...");const t=[{name:"AntiAfk",init:Ot.init.bind(Ot)},{name:"PetTeam",init:ie.init.bind(ie)},{name:"BulkFavorite",init:yo.init.bind(yo)},{name:"XPTracker",init:wo.init.bind(wo)},{name:"CropValueIndicator",init:Hn.init.bind(Hn)},{name:"CropSizeIndicator",init:Un.init.bind(Un)}];let n=0;for(const r of t)try{r.init(),n++,e.logStep("Features",`Initializing features... (${n}/${t.length})`,"info");}catch(o){e.logStep("Features",`Initializing features... (${n}/${t.length}) - ${r.name} failed`,"error"),console.warn(`[Bootstrap] Feature ${r.name} init failed`,o);}e.logStep("Features",`All features initialized (${t.length}/${t.length})`,"success"),e.logStep("Injections","Initializing QOL injections...");try{const r=Zl();r.register({id:"bulkFavoriteInject",name:"Bulk Favorite Inject",description:"Quick favorite/unfavorite multiple mutations",injection:k1,storageKey:we.BULK_FAVORITE,defaultEnabled:!1}),r.register({id:"cropValueIndicator",name:"Crop Price",description:"Shows coin value in crop tooltips",injection:Hn.render,storageKey:we.CROP_VALUE_INDICATOR,defaultEnabled:!1}),r.register({id:"cropSizeIndicator",name:"Crop Size",description:"Shows size percentage in crop tooltips",injection:Un.render,storageKey:we.CROP_SIZE_INDICATOR,defaultEnabled:!1}),r.initAll(),e.logStep("Injections","QOL injections registered and initialized","success");}catch(r){e.logStep("Injections","QOL injections initialization failed","error"),console.warn("[Bootstrap] Injections init failed",r);}}fc();bh();(async function(){kp();const e=gp({title:"Gemini is loading",subtitle:"Connecting and preparing modules..."});let t=null;try{t=P1(e),await T1(e),A1(e),I1(e),await Promise.all([(async()=>{await E1(e);})(),(async()=>{F1(e);})(),(async()=>{await L1(e);})(),(async()=>{await M1(e);})()]),e.succeed("Gemini is ready!");}catch(r){e.fail("Failed to initialize the mod.",r);}finally{t?.();}const n=await _1(e);OC({onClick:()=>n.setOpen(true)});})();
