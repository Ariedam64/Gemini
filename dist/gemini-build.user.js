...
`;class rx extends Ht{constructor(n){super({id:"tab-pets",label:"Pets"});H(this,"unsubscribeMyPets");H(this,"lastActiveTeamId",null);H(this,"teamCardPart",null);H(this,"abilityLogsCardPart",null);H(this,"deps");this.deps=n;}async build(n){this.container=n;const r=n.getRootNode();wt(r,nx,"ability-logs-card-styles");const o=this.createGrid("12px");o.id="pets",n.appendChild(o),this.initializeTeamCardPart(o),this.initializeAbilityLogsCardPart(o),this.unsubscribeMyPets=ot.myPets.subscribeStable(()=>{const i=he.getActiveTeamId();i!==this.lastActiveTeamId&&(this.lastActiveTeamId=i,this.teamCardPart?.render());}),this.lastActiveTeamId=he.getActiveTeamId();}async destroy(){this.unsubscribeMyPets&&(this.unsubscribeMyPets(),this.unsubscribeMyPets=void 0),this.teamCardPart&&(this.teamCardPart.destroy(),this.teamCardPart=null);}initializeTeamCardPart(n){this.teamCardPart||(this.teamCardPart=new tx({onTeamReordered:o=>{console.log("[PetsSection] Teams reordered:",o);},setHUDOpen:this.deps?.setHUDOpen}));const r=this.teamCardPart.build();n.replaceChildren(r),this.teamCardPart.render();}}const ox={Store:{select:ce.select.bind(ce),set:ce.set.bind(ce),subscribe:ce.subscribe.bind(ce),subscribeImmediate:ce.subscribeImmediate.bind(ce)},Globals:ot,Modules:{Version:vs,Assets:Ut,Manifest:ct,Data:de,Environment:Ke,CustomModal:pn,Sprite:ae,Tile:ut,Pixi:Ti,Audio:Sl,Cosmetic:Al},Features:{AutoFavorite:Fi,JournalChecker:Ql,BulkFavorite:Tr,Achievements:nc,Tracker:vc,AntiAfk:At,Calculators:Fl,Pets:xc,PetTeam:he},WebSocket:{chat:ib,emote:ab,wish:sb,kickPlayer:lb,setPlayerData:cb,usurpHost:db,reportSpeakingStart:ub,setSelectedGame:pb,voteForGame:fb,requestGame:mb,restartGame:gb,ping:hb,checkWeatherStatus:vb,move:bb,playerPosition:Dl,teleport:yb,moveInventoryItem:xb,dropObject:wb,pickupObject:kb,toggleFavoriteItem:zr,putItemInStorage:_i,retrieveItemFromStorage:Li,moveStorageItem:Sb,logItems:Cb,plantSeed:Ab,waterPlant:Tb,harvestCrop:Ib,sellAllCrops:Pb,purchaseDecor:Eb,purchaseEgg:Mb,purchaseTool:_b,purchaseSeed:Lb,plantEgg:Fb,hatchEgg:Ob,plantGardenPlant:Rb,potPlant:Nb,mutationPotion:$b,pickupDecor:Db,placeDecor:jb,removeGardenObject:Bb,placePet:jl,feedPet:zb,petPositions:Gb,swapPet:Bl,storePet:zl,namePet:Wb,sellPet:Hb},_internal:{getGlobals:rt,initGlobals:Rl,destroyGlobals:Yh}};function ix(){const e=_;e.Gemini=ox,e.MGSprite=ae,e.MGData=de,e.MGPixi=Ti,e.MGAssets=Ut,e.MGEnvironment=Ke;}let bo=null;function ax(){return bo||(bo=new Pf),bo}function sx(e){return [new Bd(e),new Cv,new Vy,new Hv,new rx(e)]}async function lx(){await ax().preload();}function cx(e){const{shadow:t,initialOpen:n}=e,r=v("div",{className:"gemini-panel",id:"panel",ariaHidden:String(!n)}),o=v("div",{className:"gemini-tabbar"}),i=v("div",{className:"gemini-content",id:"content"}),a=v("div",{className:"gemini-resizer",title:"Resize"}),s=v("button",{className:"gemini-close",title:"Fermer",ariaLabel:"Fermer"},"✕");r.append(o,i,a);const c=v("div",{className:"gemini-wrapper"},r);return t.append(c),{panel:r,tabbar:o,content:i,resizer:a,closeButton:s,wrapper:c}}function dx(e){const{resizer:t,host:n,shadow:r,onWidthChange:o,initialWidth:i,minWidth:a,maxWidth:s}=e;let c=a,d=s;function l(){const x=Ke.detect(),T=Math.round(_.visualViewport?.width??_.innerWidth??0);if(x.platform==="mobile"||x.os==="ios"||x.os==="android"){const b=getComputedStyle(r.host),C=parseFloat(b.getPropertyValue("--inset-l"))||0,I=parseFloat(b.getPropertyValue("--inset-r"))||0,S=Math.max(280,T-Math.round(C+I));c=280,d=S;}else c=a,d=s;return {min:c,max:d}}function u(x){return Math.max(c,Math.min(d,Number(x)||i))}function p(x){const T=u(x);n.style.setProperty("--w",`${T}px`),o(T);}l();const f=Ke.detect(),m=!(f.platform==="mobile"||f.os==="ios"||f.os==="android");let g=false;const h=x=>{if(!g)return;x.preventDefault();const T=Math.round(_.innerWidth-x.clientX);p(T);},y=()=>{g&&(g=false,document.body.style.cursor="",_.removeEventListener("mousemove",h),_.removeEventListener("mouseup",y));},k=x=>{m&&(x.preventDefault(),g=true,document.body.style.cursor="ew-resize",_.addEventListener("mousemove",h),_.addEventListener("mouseup",y));};t.addEventListener("mousedown",k);function w(){t.removeEventListener("mousedown",k),_.removeEventListener("mousemove",h),_.removeEventListener("mouseup",y);}return {calculateResponsiveBounds:l,constrainWidthToLimits:u,setHudWidth:p,destroy:w}}function ux(e){const{panel:t,onToggle:n,onClose:r,toggleCombo:o=c=>c.ctrlKey&&c.shiftKey&&c.key.toLowerCase()==="u",closeOnEscape:i=true}=e;function a(c){const d=t.classList.contains("open");if(i&&c.key==="Escape"&&d){r();return}o(c)&&(c.preventDefault(),c.stopPropagation(),n());}document.addEventListener("keydown",a,{capture:true});function s(){document.removeEventListener("keydown",a,{capture:true});}return {destroy:s}}const px=`
@layer hud {
  /* Root container */
  .gemini-wrapper {
    position: relative;
