...
`;class X_ extends vn{constructor(n){super({id:"tab-pets",label:"Pets"});U(this,"unsubscribeMyPets");U(this,"lastActiveTeamId",null);U(this,"teamCardPart",null);U(this,"abilityLogsCardPart",null);U(this,"deps");this.deps=n;}async build(n){this.container=n;const{MGSprite:o}=await ro(async()=>{const{MGSprite:i}=await Promise.resolve().then(()=>Yl);return {MGSprite:i}},void 0);await o.init();const r=n.getRootNode();_e(r,nm,"team-card-styles"),_e(r,om,"base-pet-card-styles"),_e(r,vc,"badge-styles"),_e(r,rm,"arcade-button-styles"),_e(r,tm,"gemini-icon-button-styles"),_e(r,V_,"ability-logs-card-styles");const a=this.createGrid("12px");a.id="pets",n.appendChild(a),this.initializeTeamCardPart(a),this.initializeAbilityLogsCardPart(a),this.unsubscribeMyPets=fe.myPets.subscribeStable(()=>{const i=pe.getActiveTeamId();i!==this.lastActiveTeamId&&(this.lastActiveTeamId=i,this.teamCardPart?.render());}),this.lastActiveTeamId=pe.getActiveTeamId();}async destroy(){this.unsubscribeMyPets&&(this.unsubscribeMyPets(),this.unsubscribeMyPets=void 0),this.teamCardPart&&(this.teamCardPart.destroy(),this.teamCardPart=null),this.abilityLogsCardPart&&(this.abilityLogsCardPart.destroy(),this.abilityLogsCardPart=null);}initializeTeamCardPart(n){this.teamCardPart||(this.teamCardPart=new z_({onTeamReordered:r=>{console.log("[PetsSection] Teams reordered:",r);},setHUDOpen:this.deps?.setHUDOpen}));const o=this.teamCardPart.build();n.appendChild(o),this.teamCardPart.render();}initializeAbilityLogsCardPart(n){this.abilityLogsCardPart||(this.abilityLogsCardPart=new W_);const o=this.abilityLogsCardPart.build();n.appendChild(o),this.abilityLogsCardPart.render();}}class q_{constructor(t){U(this,"root");U(this,"options");U(this,"headerElement",null);U(this,"petsContainer",null);U(this,"footerElement",null);this.options=t,this.root=document.createElement("div"),this.root.className="xp-panel";}build(){return this.headerElement=document.createElement("div"),this.headerElement.className="xp-panel__header",this.root.appendChild(this.headerElement),this.petsContainer=document.createElement("div"),this.petsContainer.className="xp-panel__pets",this.root.appendChild(this.petsContainer),this.footerElement=document.createElement("div"),this.footerElement.className="xp-panel__footer",this.root.appendChild(this.footerElement),this.root}update(t){this.updateHeader(t.teamSummary),this.updatePets(t.pets),this.updateFooter(t.teamSummary,t.pets);}updateHeader(t){this.headerElement&&(t.bonusXpPerHour>0,this.headerElement.innerHTML=`
        ...
        `;}formatHours(t){if(t===null||t===0)return "0h";if(!isFinite(t))return "∞";if(t<1)return `${Math.ceil(t*60)}m`;if(t<24)return `${t.toFixed(1)}h`;{const n=Math.floor(t/24),o=Math.floor(t%24);return `${n}d ${o}h`}}destroy(){this.root.parentNode&&this.root.parentNode.removeChild(this.root),this.headerElement=null,this.petsContainer=null,this.footerElement=null;}}const K_={id:"xp",label:"XP",icon:"📊",category:"stats",isAvailable:()=>true,getSummary:(e,t)=>{const n=kd(e.id);return n>=99?null:{text:`${Math.round(n)}%`,variant:n<33?"low":n<67?"medium":"high",tooltip:`Average progress to max STR: ${Math.round(n)}%`,priority:10}},buildPanel:(e,t)=>{const n=new q_({teamId:e.id});t.appendChild(n.build());const o=ho(e.id);return o&&n.update(o),{update:(r,a)=>{const i=ho(r.id);i&&n.update(i);},destroy:()=>n.destroy(),refresh:()=>{const r=ho(e.id);r&&n.update(r);}}},renderPetSlot:(e,t,n)=>{const o=fe.weather.get(),r=o.isActive?o.type:null,a=ho(t.id),i=a?.teamSummary.bonusXpPerHour||0,s=a?.pets||[],l=Math.max(0,...s.map(f=>f.hoursToMaxStrength||0)),d=Hs(e,r,i,l),c=d.isMaxStrength,u=!!d.xpBoostStats;let p="";if(c)u&&d.xpBoostStats&&(p=`
                ...
                `);const g=d.maxStrength,x=d.currentStrength,b=Math.min(100,Math.max(0,Math.floor(x/g*100))),h=e.xp%3600/3600*100,y=Math.min(99,Math.max(1,Math.floor(h))),_=d.currentStrength+1,k=d.maxStrength;p=f+`
                <div class="stat-row">
                    <span class="stat__label">NEXT STR</span>
                    <span class="stat__timer">${_d(d.hoursToNextStrength||0)}</span>
                ...
                <div class="stat-row">
                    ...
                    <span class="stat__timer">${_d(d.hoursToMaxStrength||0)}</span>
...
`;class vI extends vn{constructor(n){super({id:"tab-trackers",label:"Trackers"});U(this,"deps");U(this,"trackerCardPart",null);U(this,"unsubscribeMyPets");this.deps=n;}async build(n){this.container=n;const{MGSprite:o}=await ro(async()=>{const{MGSprite:i}=await Promise.resolve().then(()=>Yl);return {MGSprite:i}},void 0);await o.init(),await uI();const r=n.getRootNode();this.injectStyles(r);const a=this.createGrid("12px");a.id="trackers",n.appendChild(a),this.initializeTrackerCard(a),this.unsubscribeMyPets=fe.myPets.subscribeStable(()=>{this.trackerCardPart?.render();});}async destroy(){this.unsubscribeMyPets&&(this.unsubscribeMyPets(),this.unsubscribeMyPets=void 0),this.trackerCardPart&&(this.trackerCardPart.destroy(),this.trackerCardPart=null);}unmount(){this.destroy().catch(console.error),super.unmount();}injectStyles(n){_e(n,mI,"tracker-card-styles"),_e(n,hI,"team-card-styles"),_e(n,bI,"feature-card-styles"),_e(n,xI,"team-xp-panel-styles"),_e(n,yI,"growth-panel-styles"),_e(n,om,"base-pet-card-styles"),_e(n,vc,"badge-styles"),_e(n,rm,"arcade-button-styles"),_e(n,tm,"gemini-icon-button-styles");}initializeTrackerCard(n){this.trackerCardPart||(this.trackerCardPart=new gI({setHUDOpen:this.deps?.setHUDOpen}));const o=this.trackerCardPart.build();n.appendChild(o),this.trackerCardPart.render();}}const wI=`
  #alerts-section {
    display: flex;
    flex-direction: column;
    gap: 12px;
...
`,Vd={ui:{expandedCards:{settings:true,shops:false,weather:false,pet:false}}};async function SI(){const e=await to("tab-alerts",{version:1,defaults:Vd,sanitize:r=>({ui:{expandedCards:Na(Vd.ui.expandedCards,r.ui?.expandedCards)}})});function t(r){const a=e.get();e.update({ui:{...a.ui,...r,expandedCards:Na(a.ui.expandedCards,r.expandedCards)}});}function n(r,a){const i=e.get();e.update({ui:{...i.ui,expandedCards:{...i.ui.expandedCards,[r]:!!a}}});}function o(r){const a=e.get();n(r,!a.ui.expandedCards[r]);}return {get:e.get,set:e.set,save:e.save,setUI:t,setCardExpanded:n,toggleCard:o}}const CI={seed:"Seeds",tool:"Tools",egg:"Eggs",decor:"Decor"},Xd={seed:"🌱",tool:"🔧",egg:"🥚",decor:"🎨"},kI={seed:"plants",tool:"items",egg:"eggs",decor:"decor"},_I={seed:"seed",tool:null,egg:null,decor:null},qd={common:0,uncommon:1,rare:2,legendary:3,mythical:4,divine:5,celestial:6};function Cc(e,t,n){try{const o=kI[t],r=te.get(o);if(!r||typeof r!="object")return null;const a=r[e];if(!a||typeof a!="object")return null;const i=_I[t],s=i?a[i]:a;return !s||typeof s!="object"?null:s[n]??null}catch(o){return console.warn(`[Alerts] Failed to get ${n} for ${e}:`,o),null}}function II(e,t){return Cc(e,t,"spriteId")}function TI(e,t){const n=Cc(e,t,"rarity");return n?String(n).toLowerCase():null}function EI(e,t){return Cc(e,t,"name")??e}function PI(){const e=qt.getTrackedItems();return new Set(e.map(t=>`${t.shopType}:${t.itemId}`))}function Kd(e){const t=PI(),n=[],o=["seed","tool","egg","decor"];for(const r of o){const a=e.byType[r];if(a)for(const i of a.items){const s=`${r}:${i.id}`;n.push({...i,shopType:r,rarity:TI(i.id,r),spriteId:II(i.id,r),itemName:EI(i.id,r),isTracked:t.has(s),hasCustomSound:de.hasItemCustomSound("shop",i.id,r)});}}return n}const AI=`
  /* Container */
  .modal-container {
    position: fixed;
    top: 0;
...
`,MI={size:"md",closeOnBackdrop:true,closeOnEscape:true};function LI(e){const t={...MI,...e};let n=false,o=null,r=null,a=null,i=null;function s(){p(),t.onClose?.();}function l(y){t.closeOnBackdrop&&y.target===r&&s();}function d(y){t.closeOnEscape&&y.key==="Escape"&&s();}function c(){if(!a)return;const y=["a[href]","button:not([disabled])","textarea:not([disabled])","input:not([disabled])","select:not([disabled])",'[tabindex]:not([tabindex="-1"])'].join(","),_=Array.from(a.querySelectorAll(y));if(_.length===0)return;const k=_[0],S=_[_.length-1];k.focus();const v=w=>{w.key==="Tab"&&(w.shiftKey?document.activeElement===k&&(w.preventDefault(),S.focus()):document.activeElement===S&&(w.preventDefault(),k.focus()));};a.addEventListener("keydown",v),i=()=>{a?.removeEventListener("keydown",v);};}function u(){o=m("div",{className:"modal-container"}),o.setAttribute("role","dialog"),o.setAttribute("aria-modal","true"),o.setAttribute("aria-labelledby","modal-title");const y=m("style");y.textContent=AI,o.appendChild(y),r=m("div",{className:"modal-backdrop"}),r.addEventListener("click",l),o.appendChild(r),a=m("div",{className:`modal-dialog modal-dialog--${t.size}`});const _=m("div",{className:"modal-header"}),k=m("h2",{className:"modal-title",id:"modal-title"},t.title);_.appendChild(k);const S=m("button",{className:"modal-close",type:"button","aria-label":"Close modal"},"×");S.addEventListener("click",s),_.appendChild(S),a.appendChild(_);const v=m("div",{className:"modal-body"});if(v.appendChild(t.content),a.appendChild(v),t.footer){const w=m("div",{className:"modal-footer"});w.appendChild(t.footer),a.appendChild(w);}return r.appendChild(a),o}function p(){!n||!o||(o.classList.remove("is-open"),n=false,i&&(i(),i=null),document.removeEventListener("keydown",d),setTimeout(()=>{o?.remove();},200));}function f(){n&&p(),r?.removeEventListener("click",l),i&&(i(),i=null),document.removeEventListener("keydown",d),o?.remove(),o=null,r=null,a=null;}const g=u();return ((document.querySelector("#gemini-hud-root")||document.querySelector("#gemini-root")||document.querySelector("#gemini-hud-container"))?.shadowRoot||document.body||document.documentElement).appendChild(g),requestAnimationFrame(()=>{o?.classList.add("is-open"),n=true,c(),document.addEventListener("keydown",d);}),{root:g,close:p,destroy:f}}function dm(e={}){const{id:t,min:n=0,max:o=100,step:r=1,value:a=n,label:i,showValue:s=true,disabled:l=false,onInput:d,onChange:c}=e,u=m("div",{className:"slider"}),p=m("div",{className:"slider-row"}),f=m("div",{className:"slider-track"}),g=m("div",{className:"slider-range"});f.appendChild(g);const x=m("input",{id:t,type:"range",min:String(n),max:String(o),step:String(r),value:String(a),disabled:l});x.addEventListener("input",w=>{h(),d?.(_(),w);}),x.addEventListener("change",w=>c?.(_(),w));function b(){const w=o-n;return w===0?0:(_()-n)/w}function h(){const w=Math.max(0,Math.min(1,b()));g.style.width=`${w*100}%`,v&&(v.textContent=String(_()));}function y(w){x.value=String(w),h();}function _(){return Number(x.value)}function k(w){x.disabled=!!w;}let S=null,v=null;return i&&(S=m("span",{className:"slider-label"},i),p.appendChild(S)),f.appendChild(x),p.appendChild(f),s&&(v=m("span",{className:"slider-value"},String(a)),p.appendChild(v)),u.append(p),h(),{root:u,input:x,setValue:y,getValue:_,setDisabled:k}}const Yd=`
